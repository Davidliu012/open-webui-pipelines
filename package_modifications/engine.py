"""
    Modified from knowledge_storm/storm_wiki/engine.py
"""
import json
import logging
import os
...
class STORMWikiRunner(Engine):
    ...
    def run_knowledge_curation_module(
        ...
        # Disable the saving of conversation log and information table to local file system
    ...
    def run_outline_generation_module(
        ...
        # Disable the saving of outline to local file system
    ...
    def run_article_generation_module(
        ...
        # Disable the saving of article to local file system
    ...
    def run_article_polishing_module(
        ...
        # Disable the saving of polished article to local file system
        # FileIOHelper.write_str(
        #     polished_article.to_string(),
        #     os.path.join(self.article_output_dir, "storm_gen_article_polished.txt"),
        # )
        
        # Remeber to convert StormArticle back to string
        return polished_article.to_string()
    ...
    def run(
        ...
        if do_generate_outline:
            ...
            if information_table is None:
                raise ValueError(
                    "Step 1 Error: Information table is not available. Please check /Users/cfbiotech/miniconda3/envs/pipeline/lib/python3.11/site-packages/knowledge_storm/storm_wiki/engine.py for details."
                )
        ...
        if do_generate_article:
            ...
            #         ),
            #     )
            if information_table is None or outline is None:
                raise ValueError(
                    "Step 2 Error: Information table or outline is not available. Please check /Users/cfbiotech/miniconda3/envs/pipeline/lib/python3.11/site-packages/knowledge_storm/storm_wiki/engine.py for details."
                )
        ...
        if do_polish_article:
            ...
            if draft_article is None:
                raise ValueError(
                    "Step 3 Error: Draft article is not available. Please check /Users/cfbiotech/miniconda3/envs/pipeline/lib/python3.11/site-packages/knowledge_storm/storm_wiki/engine.py for details."
                )
        ...
        if polished_article is None:
            raise ValueError(
                "Step 4 Error: Polished article is not available. Please check /Users/cfbiotech/miniconda3/envs/pipeline/lib/python3.11/site-packages/knowledge_storm/storm_wiki/engine.py for details."
            )
